 ...
 */
 ...
 size_t binary(const std::vector<T>& elements, const C& val, int r, int l){
   unsigned m=(l+r)/2;
   if(r<l){
     return elements.size();
   }

   if(elements.size()>=2 || r>=l){
     if(m>=elements.size()){
       m=elements.size();
       return m;
     }
     ...
     if(m==0 && elements[m]>val){
       return 0;
     }
     if(m>=1 && elements[m-1]<val && elements[m]>val){
       return m;
     }
     if(val<elements[m]){
       m--;
       return binary(elements, val, m, l);
     }
     else{
       m++;
       return binary(elements, val, r, m);
     }
   }
   else{
     if(val>elements[0]){
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    r--;
    return binary(elements, val, r, l);
}
