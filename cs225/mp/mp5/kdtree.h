...
class KDTree
{
  ...
  private:
     ...
     */     vector<Point<Dim>> pt;
     void Building(int dimen, int l, int r);
     void quickselect(int dimen, int l, int r, int m);
     int partition(int dimen, int l, int r, int m);
     void swap(int lwhat, int rwhat);
     KDTree<Dim>::KDTreeNode* buildroot(int l, int r);
     Point<Dim> neighboring(const Point<Dim>& query, const Point<Dim>& currBest, int l, int r, int dimen) const;
     void deletion(KDTreeNode* subroot);
   };
