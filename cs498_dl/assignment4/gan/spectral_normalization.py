...
class SpectralNorm(nn.Module):
    ...
    def _update_u_v(self):
        ...
        #getattr(object, name[]), getattr(x, 'foobar')
        u = getattr(self.module, self.name+"_u")
        v = getattr(self.module, self.name+"_v")
        w = getattr(self.module, self.name+"_bar")
        
        for not_used in range(self.power_iterations):
            weight = torch.t(w.view(w.data.shape[0], -1).data)
            #weight = matrix | u = vector
            #v first, u second
            v.data = l2normalize(torch.mv(weight, u.data))
            weight = w.view(w.data.shape[0], -1).data
            u.data = l2normalize(torch.mv(weight, v.data))
        
        #w=u^t*w*v = sigma, W = W / sigma(W)
        weight = w.view(w.data.shape[0], -1)
