...
def train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=250, 
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            #discriminator
            D_solver.zero_grad()
            current_data = Variable(real_images).type(dtype).cuda()
            logits_real = D(current_data).type(dtype).cuda()
            fake_noise = Variable(sample_noise(batch_size, noise_size)).type(dtype).cuda()
            fake_images = G(fake_noise).detach()
